...
A [user on reddit recently asked](https://www.reddit.com/r/kde/comments/9xcin7/configure_default_plasmoid_size/) how to resize a few widgets. The "Notes" widget it pretty straight forward by changing the `Layout.preferredWidth`/`Layout.preferredHeight` as outlined in [KDE's Getting Started with Plasma Widgets](https://techbase.kde.org/Development/Tutorials/Plasma5/QML2/GettingStarted) article on the KDE wiki. However the user wanted to resize the System Tray and had no idea how to do so.
