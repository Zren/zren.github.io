...
To properly fork the widget, rename the new folder, then edit the `X-KDE-PluginInfo-Name` in the `metadata.desktop` file to match the new folder name. You will also want to edit the `Name=` in the `metadata.desktop` file as well so you can tell it apart from the widget you forked it from. It might be easier to delete all the translated `Name[fr]=` lines if you don't know your locale code. Delete the `metadata.json` if it exists (or edit it as well).
